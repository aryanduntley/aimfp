...
```
┌──────────────────────────────┐        ┌──────────────────────────────┐
│  cron / systemd / your daemon │        │  AI Assistant (a session)     │
│  runs forever; AIMFP does not │        │  present only when you are    │
└───────────────┬──────────────┘        └───────────────┬──────────────┘
                │ starts                                 │ MCP
┌───────────────▼──────────────┐        ┌───────────────▼──────────────┐
│  runner.py  (YOUR project)    │        │         MCP Server            │
│  generated by AIMFP, owned    │        │                               │
│  and editable by you          │        │  check_directive_health       │
│                               │        │  get_directive_execution_stats│
│  import aimfp.hooks  ◄────────┼── a    │  get_recent_directive_errors  │
│  get_due_directives(...)      │  library                              │
│  run_directive(...)           │  call, └───────────────┬──────────────┘
└───────────────┬──────────────┘  not RPC               │ reads
                │ writes                                 │
                └────────────►  user_directives.db  ◄────┘
...
between — it has no timer and starts nothing.
...
│   ├── security_monitor.py
│   └── runner.py           # ← the RUNNER: your scheduler calls this,
│                           #    it imports aimfp.hooks as a library
...
# ...and outside the project entirely:
/etc/cron.d/home-automation  # ← what actually STARTS runner.py. Not AIMFP.
```
...
6. AI generates a runner and wires up your scheduler (cron, systemd, or a daemon loop)
7. **Your scheduler** runs the directives — AIMFP has no timer and is not running at 3am
8. The runner records each execution through `aimfp.hooks`; logs land in
   `.aimfp-project/logs/`, statistics in `user_directives.db`
...
| **user_directive_activate** | Mark directives active (does NOT deploy — your runner does) |
| **user_directive_monitor** | Review health, statistics, and errors after the fact (read-only) |
| **user_directive_update** | Handle changes to directive source files (re-parse, re-validate) |
| **user_directive_deactivate** | Mark directives paused (does NOT stop a running process) |
...
- **AIMFP never executes a directive and never supervises the runner** — the
  generated code owns the timer and the work; AIMFP answers "what is due" and
  records "what happened" through the `aimfp.hooks` library API
- A dead runner is detected without any process inspection: the runner declares
  when it will next fire, and silence past that deadline is the evidence
